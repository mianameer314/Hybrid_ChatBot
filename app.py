...
from chatbot.sentiments import analyze
...
with st.sidebar:
    ...
    if st.button("Clear history"):
        ...
        try:
            requests.post(f"{API_URL}/clear/{session_id}")        # clear DB
            requests.delete(f"{API_URL}/clear/{session_id}")  # clear Redis
        except Exception:
            pass
        st.rerun()
    ...
    if st.button("📜 View  History"):
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                if data:
                    st.info("Cached messages:")
                    for msg in data:
                        st.write(f"**{msg['role']}**: {msg['content']}")
                else:
                    st.warning("No messages cached yet.")
            else:
                st.error(f"❌ Error: {resp.status_code} - {resp.text}")
        except Exception as e:
            st.error(f"⚠️ Could not fetch cache: {str(e)}")

    if st.button("🗑 Clear  Session"):
        try:
            resp = requests.post(f"{API_URL}/clear/{session_id}")
            if resp.status_code == 200:
                st.success("✅ Session cleared.")
            else:
                st.error(f"❌ Error: {resp.status_code} - {resp.text}")
        except Exception as e:
            st.error(f"⚠️ Could not clear session: {str(e)}")
...
if user := st.chat_input("Type a message…"):
    sa = analyze(user)
    st.session_state.messages.append({"role": "user", "content": user, "sa": sa})
